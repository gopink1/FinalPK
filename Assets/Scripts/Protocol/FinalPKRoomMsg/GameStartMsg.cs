...
{
    public GameStart gameStartData;
    public override int GetBytesNum()
    {
        int num = 8;
        num += gameStartData.CalculateSize();//获取长度
        return num;
    }
    public override byte[] Writing()
    {
        ...
        byte[] bs = ProtobufTool.GetProtoBytes(gameStartData);
    ...
    public override int Reading(byte[] bytes, int beginIndex = 0)
    {
        ...
        gameStartData = ProtobufTool.GetProtoType<GameStart>(bytes);
    ...
    public override int Reading(byte[] bytes, int dataBytesLength, int beginIndex = 0)
    {
        ...
        gameStartData = ProtobufTool.GetProtoType<GameStart>(bt);
        return index - beginIndex;
    }
